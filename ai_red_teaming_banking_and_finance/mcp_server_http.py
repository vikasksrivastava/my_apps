"""
MCP Server: Banking & Finance Tools (HTTP/SSE Transport)
=========================================================

An HTTP-based MCP server using Server-Sent Events (SSE) transport
for the banking portal chatbot.

This server exposes the same banking tools as the stdio version
but over HTTP, making it suitable for remote access and debugging.
...
Endpoints:
    ...
    POST /messages     - HTTP endpoint for MCP messages
...
MCP_SERVER_NAME = "banking-finance-tools-http"
MCP_SERVER_VERSION = "1.0.0"
MCP_SERVER_DESCRIPTION = "HTTP MCP Server for Banking & Finance Operations"
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("🏦 Banking & Finance MCP Server (HTTP/SSE)")
    print("=" * 60)
    print(f"Server: {MCP_SERVER_NAME} v{MCP_SERVER_VERSION}")
    print(f"Tools: {sum(len(v) for v in TOOL_CATEGORIES.values())}")
    print("")
    print("Endpoints:")
    print("  GET  http://localhost:8002/sse        - SSE connection")
    print("  POST http://localhost:8002/messages/  - MCP messages")
    print("=" * 60)
